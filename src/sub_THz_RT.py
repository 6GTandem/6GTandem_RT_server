...
def setup():
    ...
    logger.info("Sionna version:", sionna.rt.__version__)
...
if __name__ == "__main__":
    ...
    for ue_idx in range(ds_users.dims['user']):
        ...
        logger.info(f"Processing user {ue_idx}...")
        ...
        for stripe_idx in range(N_stripes):
            ...
            t1 = time.time()
            # log
            logger.info(f'Processing UE {ue_idx} stripe {stripe_idx} ...')
            # loop over all RUs 
        ...
        # logging
        t_end_ue = time.time()
        logger.info(f'Finished processing UE {ue_idx} in {t_end_ue-t_start_ue:.2f} seconds')
