...
if __name__ == "__main__":
    ...
    intermediate_reders = config["random_configs"][
    ...
    if intermediate_reders:
        scene.render_to_file(
            camera=my_cam, filename="empty_scene.png", resolution=[650, 500], num_samples=512, clip_at=20
        )  # Increase num_samples to increase image quality
    ...
    N_antennas = antenna_conf["N_antennas"]  # 4x1 antennas
    ...
    for ue_idx in range(ds_users.sizes['user']):
        ...
        for ue_ant_idx in range(N_antennas):
            ...
            rx = Receiver(name=f"rx_{ue_idx}", position=ue_pos_adjusted, display_radius=0.5)
                ...
                for stripe_idx in range(N_stripes):
                    ...
                    for batch_start in range(0, N_RUs, batch_size):
                        ...
                        for RU_idx in range(batch_start, batch_end):
                            # compute RU position
                            tx_pos = [
                                stripe_start_pos[0] + stripe_idx * space_between_stripses,
                            ...
                            tx_pos_adjusted[
                                0
                            ] += antenna_spacing_offset  # Apply offset to the x-coordinate (for a ULA along X)
                            tx_pos_adjusted = tx_pos_adjusted.tolist()

                            # Create RU transmitter instance
                            tx = Transmitter(
                                name=f"tx_stripe_{stripe_idx}_RU_{RU_idx}", position=tx_pos_adjusted, display_radius=0.1
                            )
                        ...
                        ]  # add ant dims back [batchsize, 1, 1, nr_subcarriers]

                        # saving results
                        for i_in_batch, RU_idx in enumerate(ru_idx_list):
                            global_idx = stripe_idx * N_RUs + RU_idx  # tx_idx in old (unbatched) code
                            channel_tensor[global_idx, ue_ant_idx, ru_ant_idx, :] = h_freq[
                                i_in_batch
                            ]  # store into channel tensor
