...
def is_point_outside_zones(
    x, y, z,
    x_zone1_start, x_zone1_stop, y_zone1_start, y_zone1_stop,
    x_zone2_start, x_zone2_stop, y_zone2_start, y_zone2_stop,
    x_zone3_start, x_zone3_stop, y_zone3_start, y_zone3_stop,
    x_zone4_start, x_zone4_stop, y_zone4_start, y_zone4_stop
):
    """ 
...
def create_user_location_dataset(config, logger):
    ...
    nr_ue_locs = num_point_zone1 + num_point_zone2 + num_point_zone3 + num_point_zone4 + N_RUs*N_stripes
    logger.info(f"testing: nr_ue_locs first compute: {nr_ue_locs}")
    ...
    if os.path.exists(file_name):    
        ...
        for stripe_idx in range(N_stripes):
            for RU_idx in range(N_RUs):
                ...
                if invalid_point:
                    logger.info(f"stripe {stripe_idx}, ru: {RU_idx}, pos: {pos} - point outside of zone: {invalid_point}")
        ...
        logger.info(f"Zone 1: area: {area_zone1} - {samples_zone1.shape[0]} samples")
        logger.info(f"Zone 2: area: {area_zone2} - {samples_zone2.shape[0]} samples")
        logger.info(f"Zone 3: area: {area_zone3} - {samples_zone3.shape[0]} samples")
        logger.info(f"Zone 4: area: {area_zone4} - {samples_zone4.shape[0]} samples")
